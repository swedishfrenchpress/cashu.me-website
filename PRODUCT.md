...
**Job to be done**: Get the right mental model (custodial backing, bearer tokens, mint-agnostic, Lightning-native, no accounts, withdraw anytime) fast enough to decide *yes, I'll try this* before the tab gets closed.
...
Success: a visitor leaves with the right mental model, bearer tokens on their device with unconditional exit to any Lightning address, and installs the wallet. Failure: they leave thinking they're locked in the way Wallet of Satoshi locks them in, or that the mint can freeze them specifically.
...
1. **Neon-on-black crypto aesthetic** — electric neon on pitch black, 3D rendered logos, futurist UI. The training-data crypto reflex. Reads as memecoin or DeFi token, not as bearer cash.
...
6. **Honest about custody, free to leave.** Cashu is custodial: mints hold the underlying Bitcoin. The site says so plainly. The value proposition is not "no custodian" (false), it's "you hold bearer tokens that withdraw to any Lightning address whenever you want, no permission, no notice." Candor about backing is more convincing than overclaiming.
...
- **Custodial-honesty rule**: copy must never claim "self-custodial" or "no custodian." Cashu is custodial; the right framing is bearer tokens plus unconditional exit. See Design Principle #6.
